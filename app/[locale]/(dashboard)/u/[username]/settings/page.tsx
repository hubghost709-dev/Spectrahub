    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-background border rounded-lg">
        ...
        >
          <div className="flex items-center gap-3">
           ...
           <h2 className="text-lg font-semibold">{t('streamTopic')}</h2>
          </div>
        ...
        {showTopic && (
          <div className="p-4 border-t">
            <div className="space-y-4">
              <Input 
                placeholder={t('streamTopicPlaceholder')}
                className="bg-muted text-white"
                value={pinnedMessage}
                onChange={(e) => setPinnedMessage(e.target.value)}
              />
              <p className="text-sm text-muted-foreground">
                {t('streamTopicDescription')}
              </p>
      ...
      <div className="bg-background border rounded-lg">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold"> {t('kingOfRoom')}</h2>
          </div>
      ...
      <div className="bg-background border rounded-lg">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold"> {t('collectMoney')} </h2>
          </div>
      ...
      <div className="bg-background border rounded-lg">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold"> {t('hideProfile')}  </h2>
          </div>
        ...
        {showHideProfile && (
          <div className="p-4 border-t space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium"> {t('hideProfileLabel')}  </label>
              <Select
                value={blockedCountries.join(",")}
                onValueChange={(value) => handleUpdateBlockedCountries(value ? value.split(",") : [])}
              >
                <SelectTrigger>
                  <SelectValue placeholder={t('hideProfileLabels')} />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-sm text-muted-foreground">
                {t('hideProfileDescription')} 
              </p>
              {blockedCountries.length > 0 && (
                <div className="mt-4">
                  <h3 className="text-sm font-medium mb-2">  {t('hiddenIn')}  </h3>
      ...
      <div className="bg-background border rounded-lg">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold">     {t('tipMenu')} </h2>
          </div>
          ...
          <div className="p-4 border-t space-y-4">
            <div className="space-y-4">
              <div className="grid gap-4">
                <div>
                  <label className="text-sm font-medium"> {t('tipName')} </label>
                  <Input
                ...
                <div>
                  <label className="text-sm font-medium"> {t('tipDescription')} </label>
                  <Input
                    value={newTipItem.description}
                    onChange={(e) => setNewTipItem(prev => ({ ...prev, description: e.target.value }))}
                    placeholder={t('tipDescriptionPlaceholder')} 
                  />
                </div>
                <div>
                  <label className="text-sm font-medium">{t('tipTokens')} </label>
                  <Input
                    type="number"
                    value={newTipItem.tokens}
                    onChange={(e) => setNewTipItem(prev => ({ ...prev, tokens: e.target.value }))}
                    placeholder={t('tipTokensm')}
                  />
                </div>
              </div>
              <Button
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-medium"> {t('tipcurrent')}   </h3>
              {tipMenuItems.map((item) => (
                <div key={item.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  <div>
                    ...
                    <p className="text-sm">{item.tokens} {t('tipTokens')}</p>
                  </div>
                  <Button
                    variant="destructive"
                    size="sm"
      ...
      <div className="bg-background border rounded-lg">
        ...
        >
          <div className="flex items-center gap-3">
            <Video className="h-5 w-5" />
            <h2 className="text-lg font-semibold"> {t('Recording')}</h2>
          </div>
        ...
        {showRecording && (
          <div className="p-4 border-t space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium"> {t('Recordingallow')}  </p>
                <p className="text-sm text-muted-foreground">
                  {t('Recordingset')}
                </p>
              </div>
